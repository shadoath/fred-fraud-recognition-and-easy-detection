import { offlineCheckEmailForFraud, offlineCheckTextForFraud } from "../lib/offlineFraudService"
  ...
  describe("offlineCheckEmailForFraud", () => {
    ...
    it("should detect suspicious sender domain", async () => {
      const emailData: EmailData = {
        ...
        content:
          "Your bank account requires verification due to suspicious activity. Click here to verify.",
        timestamp: new Date().toISOString(),
      }
    ...
    it("should detect multiple suspicious indicators", async () => {
      const emailData: EmailData = {
        ...
        content:
          "Dear customer, we have detected suspicious activity on your account. Click here to verify your account immediately: https://fake-bank.com/verify. You must act now!!! $$$",
        timestamp: new Date().toISOString(),
      }
    ...
    it("should consider safe emails as low threat", async () => {
      const emailData: EmailData = {
        ...
        content:
          "Hi team, attached are the meeting notes from our discussion yesterday. Let me know if you have any questions.",
        timestamp: new Date().toISOString(),
      }
  ...
  describe("offlineCheckTextForFraud", () => {
    it("should detect get-rich-quick schemes", async () => {
      const textData: TextData = {
        content:
          "Make money fast with this guaranteed investment. Risk free investment with HUGE returns!",
        timestamp: new Date().toISOString(),
      }
    ...
    it("should detect requests for personal information", async () => {
      const textData: TextData = {
        content:
          "To continue with your application, we need your social security number and mother's maiden name.",
        timestamp: new Date().toISOString(),
      }
    ...
    it("should detect multiple URLs as suspicious", async () => {
      const textData: TextData = {
        content:
          "Check out these great deals: https://site1.com, https://site2.com, https://site3.com, https://site4.com",
        timestamp: new Date().toISOString(),
      }
    ...
    it("should consider normal text as low threat", async () => {
      const textData: TextData = {
        content:
          "Hi, I wanted to follow up on our conversation from yesterday. When would be a good time to meet and discuss the project timeline?",
        timestamp: new Date().toISOString(),
      }
