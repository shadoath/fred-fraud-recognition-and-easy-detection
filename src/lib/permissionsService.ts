 ...
 * Returns true if permission was granted, false otherwise
 */
export async function requestPermissionWithPrompt(url: string, reason: string): Promise<boolean> {
  ...
  const confirmMessage = 
    `FRED needs permission to analyze content from "${hostname}" ${reason ? `for ${reason}` : ''}\n\n` + 
 ...
 */
export async function getGrantedHostPermissions(): Promise<string[]> {
  try {
