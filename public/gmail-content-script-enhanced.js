...
async function checkSitePermission() {
  try {
    ...
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({ action: "checkPermission", url }, (response) => {
...
async function requestSitePermission() {
  try {
    ...
    return new Promise((resolve) => {
      chrome.runtime.sendMessage({ action: "requestPermission", url }, (response) => {
        if (response && response.success) {
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            ...
            if (response && response.success) {
              ...
              showNotification(
                "Error analyzing email: " + (response?.error || "Unknown error"),
                "error"
              )
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 16px; border-bottom: 1px solid #eee;">
      <div style="display: flex; align-items: center;">
        <img src="${chrome.runtime.getURL(
          "fred-48.png"
        )}" width="24" height="24" alt="FRED Logo" style="margin-right: 8px;">
...
function setupObserver() {
  ...
  console.log(
    `FRED - Observer set up on ${
      possibleTargets.length
    } targets for Gmail ${detectGmailVersion()} version`
  )
    ...
    if (isEmailOpen) {
      ...
      if (hasPermission) {
        ...
        const toolbarElement =
          document.querySelector("[data-message-id] .G-tF") ||
